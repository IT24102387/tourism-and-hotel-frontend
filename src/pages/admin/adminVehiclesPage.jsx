...
const VEHICLE_TYPES = ["Mahindra Jeep", "Toyota Hilux", "Safari Jeep", "Land Cruiser", "Minibus", "Van"];
...
export default function AdminVehiclesPage() {
  ...
  const fetchVehicles = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/vehicles`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingVehicle) {
        await axios.put(
          `${import.meta.env.VITE_BACKEND_URL}/api/vehicles/${editingVehicle.vehicleId}`,
          payload,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        toast.success("Vehicle updated successfully");
      } else {
        await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/vehicles`, payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
    ...
    <div className="min-h-screen bg-gray-100 p-4 md:p-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div className="flex items-center gap-3">
          <FaCar className="text-[#2F2D8F]" style={{ fontSize: 28 }} />
          <h1 className="text-3xl font-bold text-gray-800">Transportation</h1>
        </div>
        <button
          onClick={openAdd}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl shadow-lg p-12 text-center">
          <FaCar className="mx-auto text-gray-300 mb-4" style={{ fontSize: 64 }} />
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <FormField label="Capacity (passengers) *">
                  <input
                    type="number"
                    min="1"
                    step="1"
              ...
              {/* Row 3: Price + Status */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <FormField label="Price Per Day (LKR) *">
                  <input
                    type="number"
                    min="1"
                    step="1"
